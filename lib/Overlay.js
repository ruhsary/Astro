...
Overlay = (function() {
  ...
  Overlay.prototype.request = function(req) {
    ...
    if ((this.buffer[req.x] != null) && (this.buffer[req.x][req.y] != null)) {
      ...
      return this.requestImage(x, y, this.scale, cb);
    }
  };
  ...
  Overlay.prototype.requestFIRST = function(degX, degY, scale, cb) {
    var decMax, decMin, imgURL, raMax, raMin, url;
    decMin = degY - .256;
    decMax = degY + .256;
    raMax = degX + .256;
    raMin = degX - .256;
    ...
    $.get(url, {
      ...
      DecMin: decMin,
      DecMax: decMax
    }, function(data) {}, imgURL = "", data[0] ? imgURL = this.imagePath + data[0] : imgURL = this.placeholder, cb(imgURL), this.view.display());
    return $.get(url, {
      RAMin: raMin,
      RAMax: raMax,
      DecMin: decMin,
